...
namespace MonsterLegendsLite {
    public class Shop_MonsterList : Shop_ItemList {
        protected override IEnumerable<(IShopItemData, IShopItemLocData)> IEAllItemData() {
            var datas    = DataManager.Ins.GameDefData.Monster;
            var locDatas = DataManager.Ins.GameLocDefData.Monster;
        ...
        protected override void DoBuyLogic(IShopItemData data, IShopItemLocData locData) {
            Home_BootData.InsAutoSpawn.SetData_BuyMonster(DataManager.Ins.GameDefData.Monster.First(i => i.Value == data).Key);
