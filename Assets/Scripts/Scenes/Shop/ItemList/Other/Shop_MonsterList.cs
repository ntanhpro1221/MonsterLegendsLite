...
namespace MonsterLegendsLite {
    public class Shop_MonsterList : Shop_ItemList {
        protected override IEnumerable<(IShopItemData, IShopItemLocData)> IEAllItemData() {
            var datas    = DataManager.Ins.GameDef.Monsters;
            var locDatas = DataManager.Ins.GameLocDef.Monsters;
        ...
        protected override void DoBuyLogic(IShopItemData data, IShopItemLocData locData) {
            Home_BootData.InsAutoSpawn.SetData_BuyMonster(DataManager.Ins.GameDef.Monsters.First(i => i.Value == data).Key);
        ...
        protected override bool IsCanBuy(IShopItemData data, IShopItemLocData locData, out string blockReason) {
            if (!DataManager.Ins.IsAnyHabitatCanAcceptNewMonster(new MonsterInsData(DataManager.Ins.GameDef.Monsters.First(i => i.Value == data).Key))) {
